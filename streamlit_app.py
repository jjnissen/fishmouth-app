...
def draw_concept_visual(mode, h_od, b_od, offset=0):
    ...
    ax.axis('off')
    if mode == "ECCENTRIC":
        header = plt.Circle((0, 0), h_od/2, facecolor='white', edgecolor='#0e3c61', lw=2)
        ax.add_patch(header)
        ax.plot([0, 0], [-h_od/1.6, h_od/1.6], color='#0e3c61', linestyle='-.', lw=0.5)
        ax.plot([-h_od/1.6, h_od/1.6], [0, 0], color='#0e3c61', linestyle='-.', lw=0.5)
        branch = plt.Circle((0, offset), b_od/2, facecolor='#d9eaf7', edgecolor='#2196f3', lw=2, alpha=0.9)
        ax.add_patch(branch)
        if offset > 0:
             ax.annotate('', xy=(0, 0), xytext=(0, offset), arrowprops=dict(arrowstyle='<-', color='red', lw=2))
        ax.set_xlim(-h_od/1.4, h_od/1.4); ax.set_ylim(-h_od/1.4, h_od/1.4)
    return fig

def draw_install_guide():
    """Draws the diagram from Page 27 showing Base Line & Measurements"""
    fig, ax = plt.subplots(figsize=(6, 3))
    
    # Draw Pipe (Side View)
    rect = patches.Rectangle((0, 0), 6, 3, linewidth=2, edgecolor='#0e3c61', facecolor='white')
    ax.add_patch(rect)
    ...
    # Draw Ordinate Lines
    x_pos = np.linspace(0.5, 5.5, 9)
    heights = [1.0, 1.2, 1.5, 2.0, 2.5, 2.0, 1.5, 1.2, 1.0] # Fake curve for visual
    
    for i, x in enumerate(x_pos):
        # Draw Line up
        ax.vlines(x, 0.5, 0.5 + heights[i], color='#2196f3', lw=1)
        # Draw Arrow
        ax.annotate('', xy=(x, 0.5 + heights[i]), xytext=(x, 0.5), arrowprops=dict(arrowstyle='->', color='#2196f3', lw=1.5))
        # Draw Number
        ax.text(x, 0.1, f"#{i+1}", ha='center', fontsize=8, fontweight='bold')

    # Curve
    ax.plot(x_pos, [h + 0.5 for h in heights], color='#b71c1c', linewidth=2)
    ax.text(3, 3.2, "CUT LINE", ha='center', color='#b71c1c', fontweight='bold')
    
    ax.set_xlim(-0.5, 6.5); ax.set_ylim(0, 3.5)
    ax.axis('off')
    return fig
...
if st.session_state.step == 1:
    ...
    with c2:
        ...
        if st.button("❓ App Instructions"):
            st.session_state.tool = "Help"; st.session_state.step = 2; st.rerun()
...
# ==============================================================================
    ...
    if st.session_state.tool == "Fishmouth":
        ...
        c1, c2 = st.columns(2)
        with c1:
            h_nom = st.selectbox("Header", all_sizes, index=12)
        with c2:
            valid_branches = all_sizes[:all_sizes.index(h_nom)+1]
            b_nom = st.selectbox("Branch", valid_branches, index=len(valid_branches)-1)
    ...
    # (Brief Logic for other tools)
        ...
        st.markdown("### 📲 How to Install on Phone")
        st.info("This is a Web App. You don't need the App Store.")
        st.write("**On iPhone (Safari):**")
        st.write("1. Tap the **Share** button (Square with arrow up).")
        st.write("2. Scroll down and tap **'Add to Home Screen'**.")
        st.divider()
        st.write("**On Android (Chrome):**")
        st.write("1. Tap the **three dots** (top right).")
        st.write("2. Tap **'Add to Home screen'**.")
...
# ==============================================================================
    ...
    if st.session_state.tool == "Fishmouth":
        ...
        # --- INSTALLATION DIAGRAM (THE REQUESTED IMAGE) ---
        st.markdown("### 📋 Installation Guide")
        st.pyplot(draw_install_guide())
        
        st.markdown(f"""
        <div class="instruction-box">
            <b>1. Draw Base Line:</b> Mark a ring around your {d['b_nom']}" pipe.<br>
            <b>2. Divide:</b> Mark 16 equal sections along the line.<br>
            <b>3. Measure UP:</b> Use the table below to mark the cut points.
        </div>
        """, unsafe_allow_html=True)

        st.markdown("### ✂️ Cutting Template")
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.fill_between(x, y, max(y)*1.15, color='#ffcdd2', label="Waste")
        ax.fill_between(x, 0, y, color='#e1f5fe', label="Pipe")
        ax.plot(x, y, color='#b71c1c', lw=3)
        ax.axhline(0, color='black', lw=2)
        
        # Draw Arrows & Numbers
        for i in range(0, 33, 4):
            ax.vlines(x[i], 0, y[i], color='black', lw=0.5)
            ax.plot(x[i], y[i], marker='v', color='black', markersize=4)
            ax.text(x[i], -0.12*max(y), f"{int(i/2)+1}", ha='center', fontsize=9, fontweight='bold', 
                    bbox=dict(facecolor='white', edgecolor='#2196f3', boxstyle='circle,pad=0.2'))

        ax.set_xlim(-1, max(x)+1); ax.set_ylim(-0.25*max(y), max(y)*1.25); ax.axis('off')
        st.pyplot(fig)
    ...
    # (Results for Lobster/Miter kept simple)
    elif st.session_state.tool == "Lobster":
        # ... [Reuse Math] ...
        d = st.session_state.inputs
        ...
        st.success(f"Cut {d['pieces']-2} middle pieces and 2 end pieces.")
        c1, c2, c3 = st.columns(3)
        c1.metric("Angle", f"{round(miter_angle, 1)}°")
        c2.metric("Long", f"{round(long, 3)}\"")
        c3.metric("Short", f"{round(short, 3)}\"")
        ...
        st.caption("Mark ring. Measure this distance.")
